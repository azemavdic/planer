...
const Card = ({ racun, refs }) => {
    ...
    const { vodaZadnji } = useGetVodaQuery(undefined, {
        selectFromResult: ({ data }) => ({
            vodaZadnji:
                data?.user?.voda.length > 0 ? data?.user?.voda[0].mjesec : '',
        }),
    });
    const { smeceZadnji } = useGetSmeceQuery(undefined, {
        selectFromResult: ({ data }) => ({
            smeceZadnji:
                data?.user?.smece.length > 0 ? data?.user?.smece[0].mjesec : '',
        }),
    });
    ...
    const { iptvZadnji } = useGetIptvQuery(undefined, {
        selectFromResult: ({ data }) => ({
            iptvZadnji:
                data?.user?.iptv.length > 0 ? data?.user?.iptv[0].mjesec : '',
        }),
    });
